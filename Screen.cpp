...
void Layar::Screen::Start(string windowTitle, unsigned int screenWidth, unsigned int screenHeight, bool vsync, WindowFlag windowFlag, unsigned int targetFrameRate, float timeScale)
{
	//Initialize SDL
...
float Layar::Screen::GetDeltaTime() {
...
void Layar::Screen::OpenGameController() {
...
void Layar::Screen::CloseGameController() {
...
void Layar::Screen::PressKey(unsigned int keyID) {
...
void Layar::Screen::ReleaseKey(unsigned int keyID) {
	auto it = _mapNames.find(keyID);
	if (it != _mapNames.end()) {
		string mapName = it->second;
...
void Layar::Screen::SetMouseCoords(float x, float y) {
	_mouseCoords.x = x;
	_mouseCoords.y = y;
}

bool Layar::Screen::IsKeyDown(string name) {
...
bool Layar::Screen::IsKeyUp(string name) {
...
bool Layar::Screen::WasKeyDown(string name) {
	// We dont want to use the associative array approach here
	// because we don't want to create a key if it doesnt exist.
	// So we do it manually
...
void Layar::Screen::InputMapping(string mappingName, unsigned int keyId)
{
	_mapNames.insert(pair<unsigned int, string>(keyId, mappingName));
}

//Prints out an error message and exits the game
void Layar::Screen::Err(string errorString)
{
...
void Layar::Screen::CheckShaderErrors(GLuint shader, string type)
{
	GLint success;
	GLchar infoLog[1024];
	if (type != "PROGRAM")
...
GLuint Layar::Screen::BuildShader(const char* vertexPath, const char* fragmentPath, const char* geometryPath)
{
...
void Layar::Screen::UseShader(GLuint program)
{
	// Uses the current shader
	glUseProgram(program);
}

unsigned int Layar::Screen::GetScreenHeight() {
	return this->screenHeight;
}

unsigned int Layar::Screen::GetScreenWidth() {
